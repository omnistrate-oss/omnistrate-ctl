...
type HelmPackageInfo struct {
	...
	ChartValues   map[string]any `json:"chart_values,omitempty"`
}
...
type DeploymentCellAmenitiesStatus struct {
	...
	CurrentConfiguration map[string]any `json:"current_configuration,omitempty"`

	// Target configuration from organization template
	TargetConfiguration map[string]any `json:"target_configuration,omitempty"`
...
type ConfigurationDrift struct {
	...
	CurrentValue any    `json:"current_value"` // Current value in deployment cell
	TargetValue  any    `json:"target_value"`  // Expected value from organization template
...
type PendingConfigurationChange struct {
	...
	OldValue  any    `json:"old_value,omitempty"` // Current value (for update/delete)
	NewValue  any    `json:"new_value,omitempty"` // New value (for add/update)
}
