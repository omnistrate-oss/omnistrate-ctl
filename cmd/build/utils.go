...
// --load is omitted only when multiple platforms are specified (--load is
// incompatible with multi-platform builds). For single-platform builds, --load
// is always included so the image is available locally for pushing.
