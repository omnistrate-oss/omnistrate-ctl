...
func runBuildFromRepo(cmd *cobra.Command, args []string) error {
	...
	if !composeSpecExists || composeSpecHasBuildContext {
		...
		if !composeSpecExists {
			...
			fileData = []byte(strings.ReplaceAll(string(fileData), pat, githubPAT))
...
`, ghUsername))...)
	...
	if strings.Contains(string(fileData), githubPAT) && pat == "" {
		sm, pat, err = getOrCreatePAT(sm, resetPAT)
		if err != nil {
			utils.HandleSpinnerError(spinner, sm, err)
			return err
	...
	if strings.Contains(string(fileData), githubPAT) {
		fileData = []byte(strings.ReplaceAll(string(fileData), githubPAT, pat))
	}
