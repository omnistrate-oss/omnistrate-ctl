...
func TestHelmTabConstants(t *testing.T) {
	...
	require.Equal(1, helmTabAppLogs)
	require.Equal(2, helmTabValues)
	require.Equal(3, helmTabInputVars)
	require.Equal(4, helmTabOutputVars)
	require.Equal(5, helmTabWfErrors)
	require.Equal(6, helmNumTabs)
	require.Len(helmTabNames, helmNumTabs)
	require.Equal("App Logs", helmTabNames[helmTabAppLogs])
