...
func runModify(cmd *cobra.Command, args []string) error {
	...
	if len(param) == 0 && len(paramFile) == 0 && len(networkType) == 0 && !tagsProvided {
		err = errors.New("at least one of --param, --param-file, --network-type or --tags must be provided")
		utils.PrintError(err)
		return err
	}
	...
	if output != "json" && waitFlag {
		...
		workflowResult, workflowErr := DisplayWorkflowResourceDataWithSpinners(cmd.Context(), token, formattedInstance.InstanceID, "modify")
		err = workflowErr
		if err != nil {
